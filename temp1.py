...
heuristic = {'S': 8, 'A': 8, 'B': 1, 'C': 3, 'D': 5000, 'E': 5000, 'G': 0}
...
def deepSearch():
    ...
    while True:
        ...
        for item in tree[node]:
            if item[0] in [closed_item[0] for closed_item in closed]:
                continue
            heuristic.update({item[0]: heuristic[node] + item[1]})  # add nodes to cost dictionary
            fn_node =  heuristic[item[0]] + item[1]  # calculate f(n) of current node
            temp = [item[0], fn_node]
    ...
    for i in range(len(closed) - 2, -1, -1):
        ...
        if trace_node in [children[0] for children in tree[check_node]]:
            ...
            if heuristic[check_node] + children_costs[children_nodes.index(trace_node)] == heuristic[trace_node]:
...
if __name__ == '__main__':
    ...
    visited_nodes, optimal_nodes = deepSearch()
    print('visited nodes greedy: ' + str(visited_nodes))
    print('optimal nodes sequence greedy: ' + str(optimal_nodes))
